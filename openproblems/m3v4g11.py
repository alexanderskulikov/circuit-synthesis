...
#### MOD3 SPECIFIC
def residuevarnum(r1, r2, res):
  assert res in [0, 1, 2, 17] and r1 in [0, 1] and r2 in [0, 1]
  return varnum("res_" + str(res) + "_" + str(r1) + "_" + str(r2))

# each pair (a,b) encodes some residue
for (a,b) in it.product(range(0,2), repeat=2):
    exactly_one_of([residuevarnum(a, b, res) for res in [0, 1, 2, 17]])

# each residue is encoded somehow
for res in range(3):
    clauses.append([residuevarnum(a, b, res) for (a,b) in it.product(range(0,2), repeat=2)])
...
for t in range(0, 1 << n):
    ...
    for res1 in range(3):
      for res2 in [0, 1, 2, 17]:
        if (res1 + x1 + x2 + x3 + x4) % 3 == res2:
          continue

        for (i, j) in it.product(range(n + 1, n + r + 1), repeat=2):
          clauses.append([-residuevarnum(r1, r2, res1), -outputvarnum(1, i), -outputvarnum(2, j),
                          gatevalue(i, t), gatevalue(j, t), -residuevarnum(0, 0, res2)])
          clauses.append([-residuevarnum(r1, r2, res1), -outputvarnum(1, i), -outputvarnum(2, j),
                          -gatevalue(i, t), gatevalue(j, t), -residuevarnum(1, 0, res2)])
          clauses.append([-residuevarnum(r1, r2, res1), -outputvarnum(1, i), -outputvarnum(2, j),
                          gatevalue(i, t), -gatevalue(j, t), -residuevarnum(0, 1, res2)])
          clauses.append([-residuevarnum(r1, r2, res1), -outputvarnum(1, i), -outputvarnum(2, j),
                          -gatevalue(i, t), -gatevalue(j, t), -residuevarnum(1, 1, res2)])
...
#    for a in range(2):
#        for b in range(2):
#                if vars["res_"+str(r)+"_"+str(a)+"_"+str(b)] in satass:
#                    print(a, b, "->", r)
